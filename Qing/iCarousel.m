...
- (void)setUp
{
    ...
    perspective = -1.0f/500.0f; // 透视
    decelerationRate = 0.95f; // 放手松开的速率
    scrollEnabled = YES;
    bounces = YES; // 是否弹跳
    scrollOffset = 0.0f;
    ...
    scrollToItemBoundary = YES; //  滚动到边界
    useDisplayLink = YES;
    ignorePerpendicularSwipes = YES;  //忽略垂直的
...
- (void)scrollByNumberOfItems:(NSInteger)itemCount duration:(NSTimeInterval)duration
{
	if (duration > 0.0)
    {
        ...
        if (itemCount > 0)
        {
            ...
            NSLog(@"itemwidth %f itemCount %d",itemWidth,itemCount);
        }
        else if (itemCount < 0)
        {
